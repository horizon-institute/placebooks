...
public class PlaceBookCanvas extends FlowPanel
{
	...
	public PlaceBookItemWidget add(PlaceBookItem item)
	{
		placebook.add(item);
		return addToCanvas(item);
	}
	
	private PlaceBookItemWidget addToCanvas(PlaceBookItem item)
	{
	...
	{		
		...
		if(this.placebook == null)
		{
			...
			{
				final PlaceBookPanel panel = new PlaceBookPanel(index, DEFAULT_COLUMNS);
				panels.add(panel);
				add(panel);
			}
		}
