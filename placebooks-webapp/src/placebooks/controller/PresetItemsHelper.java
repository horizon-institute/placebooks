...
public class PresetItemsHelper
{
	...
	public static ArrayList<placebooks.model.PlaceBookItem> getPresetItems(User user)
	{
		ArrayList<PlaceBookItem> items = new ArrayList<PlaceBookItem>();
	...
	protected static void deserialiseAndAdd(String json, ArrayList<PlaceBookItem> items)
	{
		...
		try
		{
			PlaceBookItem item = mapper.readValue(json, PlaceBookItem.class);
			log.debug("Adding Preset Item: " + item.getMetadataValue("title"));
			items.add(item);
		}
		catch(Exception ex)
		{
